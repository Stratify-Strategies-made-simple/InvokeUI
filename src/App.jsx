...
import { 
  LayoutGrid, FileText, Image as ImageIcon, Music, Tag, 
  ChevronRight, Bookmark, Heart, Sparkles, Copy, Trash2, 
  Plus, Search, FileSpreadsheet, Settings, Zap, Box, 
  Database, LogOut, Check, X, MessageCircle, Send, Globe, Lock, Pencil,
  Shield, Users, Lightbulb, ArrowRight
} from 'lucide-react';
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-scale-in">
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          <div>
            ...
            <textarea name="content" required rows="4" placeholder="Enter your prompt here..." className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all" />
          </div>
...
const ManageCategoriesModal = ({ isOpen, onClose, categories, onUpdate, onDelete, user, hiddenCategoryNames, onToggleVisibility }) => {
...
const SpaceCard = ({ card, user, savedItems, toggleSave, handleDeleteClick, setSelectedSpace }) => {
...
export default function App() {
  ...
  const toggleCategoryVisibility = (categoryName) => {
    setHiddenCategoryNames(prev => {
      ...
      localStorage.setItem('invoke_hidden_categories', JSON.stringify([...newSet]));
      return newSet;
    });
  };
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snapshot) => {
      let newSpaces = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snapshot) => {
      const customCats = snapshot.docs.map(doc => ({
        id: doc.id, name: doc.data().name, icon: Tag, createdBy: doc.data().createdBy, isMain: false
      }));
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, "public_spaces", selectedSpace.id, "prompts"));
    return onSnapshot(q, (snapshot) => {
  ...
  const handleCreateSpace = async (e) => {
    e.preventDefault();
    if (!user) return setNotification({ message: 'Please sign in', type: 'info' });
    ...
    } catch (error) { setNotification({ message: 'Error creating space', type: 'info' }); }
  };

  const handleCreateCategory = async (e) => {
    e.preventDefault();
    if (!user) return setNotification({ message: 'Sign in to add categories', type: 'info' });
    const formData = new FormData(e.target);
    const categoryName = formData.get('name');
    if (categories.some(c => c.name.toLowerCase() === categoryName.toLowerCase())) return setNotification({ message: 'Category already exists', type: 'info' });
    try {
        await addDoc(collection(db, "public_categories"), { name: categoryName, createdAt: new Date().toISOString(), createdBy: user.uid });
  ...
  const handleAddPrompt = async (e) => {
    ...
    try {
      await addDoc(collection(db, "public_spaces", selectedSpace.id, "prompts"), {
        content: new FormData(e.target).get('content'),
        author: user.displayName || 'Anonymous', userId: user.uid, createdAt: new Date().toISOString()
      });
  ...
  const handleLike = async (space) => {
    if (!user) return setNotification({ message: 'Please sign in to like', type: 'info' });
    ...
    try {
      ...
      const newLikes = wasLiked ? (space.likes || 0) - 1 : (space.likes || 0) + 1;
      const newLikedBy = wasLiked ? (space.likedBy||[]).filter(id=>id!==user.uid) : [...(space.likedBy||[]), user.uid];
      setCards(prev => prev.map(c => c.id === space.id ? { ...c, likes: newLikes, likedBy: newLikedBy } : c));
      if (selectedSpace?.id === space.id) setSelectedSpace(prev => ({ ...prev, likes: newLikes, likedBy: newLikedBy }));
      await updateDoc(spaceRef, {
        likes: increment(wasLiked ? -1 : 1),
        likedBy: wasLiked ? arrayRemove(user.uid) : arrayUnion(user.uid)
      });
    } finally { processingLikes.current.delete(space.id); }
  };

  const toggleSave = async (card) => {
    if (!user) return setNotification({ message: 'Log in to save items', type: 'info' });
    ...
    if (isSaved) {
       ...
       await setDoc(docRef, { ...card, savedAt: new Date().toLocaleDateString() });
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (itemToDelete.type === 'prompt') {
        await deleteDoc(doc(db, "public_spaces", selectedSpace.id, "prompts", itemToDelete.id));
        ...
        if (selectedSpace?.id === itemToDelete.id) { setCurrentView('spaces'); setSelectedSpace(null); }
      }
    } catch (e) { setNotification({ message: 'Error deleting', type: 'info' }); }
  ...
  const filteredCards = useMemo(() => {
    return cards.filter(card => {
      if (searchQuery) {
         ...
         if (!card.title.toLowerCase().includes(q) && !card.description.toLowerCase().includes(q) && !card.author.toLowerCase().includes(q)) return false;
      }
      if (selectedCategory !== 'All' && card.category !== selectedCategory) return false;
      if (card.visibility === 'public' || !card.visibility) return true;
      if (card.visibility === 'private' && user && card.userId === user.uid) return true;
      return false; 
    });
  }, [cards, selectedCategory, user, searchQuery]);

  const visibleCategories = categories.filter(c => !hiddenCategoryNames.has(c.name));
              ...
              <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-br ${selectedSpace.gradient} text-white shadow-xl mb-8`}>
                <div className="relative p-8 md:p-12">
                   ...
                   <div className="flex items-center gap-6 mt-8 pt-8 border-t border-white/10">
                      <div className="flex items-center gap-3">
                         <img src={selectedSpace.authorImage || `https://api.dicebear.com/7.x/avataaars/svg?seed=${selectedSpace.author}`} className="w-10 h-10 rounded-full border-2 border-white/20 bg-white/10" alt="author" />
                         <div>
                      ...
                      <button onClick={() => handleLike(selectedSpace)} className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors border border-white/10">
                         <Heart className={`w-4 h-4 ${selectedSpace.likedBy?.includes(user?.uid) ? 'fill-red-500 text-red-500' : 'text-white'}`} />
                         <span className="font-bold">{selectedSpace.likes || 0}</span>
                      </button>
                   </div>
                </div>
              </div>
